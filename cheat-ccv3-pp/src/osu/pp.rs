...
impl OsuPpInner {
    fn calculate(mut self) -> OsuPerformanceAttributes {
        ...
        let miss_combo_mult = if miss_count <= 0.0 {
            ...
            self.cc_v3_strain_aware_miss_mult(miss_count)
        };

        let four_mod_tax = if is_4mod && (self.attrs.max_combo < 300) {
    ...
    fn cc_v3_strain_aware_miss_mult(&self, miss_count: f64) -> f64 {
